"""Run local-DP FedAvg simulations and report the privacy-utility tradeoff."""

from __future__ import annotations

import argparse
import json
from pathlib import Path

from ci3203_federated.fedavg import (
    label_distribution,
    make_iid_partitions,
    make_label_skew_partitions,
    train_one_client,
)
from ci3203_federated.privacy import aggregate_private_updates
from ci3203_federated.run_fedavg import evaluate_model, maybe_subsample_training
from shared.config import DatasetConfig, TrainingConfig
from shared.data import prepare_from_disk
from shared.model import build_mlp, require_torch


def run_private_setting(
    setting: str,
    train_features,
    train_labels,
    test_features,
    test_labels,
    num_clients: int,
    rounds: int,
    local_epochs: int,
    batch_size: int,
    learning_rate: float,
    clip_norm: float,
    noise_multiplier: float,
    seed: int,
) -> dict[str, object]:
    """Run FedAvg where clients send clipped and noised updates."""

    torch, _ = require_torch()
    torch.manual_seed(seed)
    generator = torch.Generator().manual_seed(seed)

    if setting == "iid":
        shards = make_iid_partitions(len(train_labels), num_clients, seed)
    elif setting == "non_iid_label_skew":
        shards = make_label_skew_partitions(train_labels, num_clients, seed)
    else:
        raise ValueError(f"Unknown setting: {setting}")

    global_model = build_mlp(
        input_dim=train_features.shape[1],
        hidden_dims=TrainingConfig().hidden_dims,
        dropout=TrainingConfig().dropout,
    )

    history = []
    unstable = False
    for round_index in range(1, rounds + 1):
        global_state = global_model.state_dict()
        local_states = []
        client_sizes = []

        for shard in shards:
            local_states.append(
                train_one_client(
                    global_model=global_model,
                    features=train_features,
                    labels=train_labels,
                    shard=shard,
                    local_epochs=local_epochs,
                    batch_size=batch_size,
                    learning_rate=learning_rate,
                )
            )
            client_sizes.append(shard.size)

        private_state = aggregate_private_updates(
            global_state=global_state,
            local_states=local_states,
            client_sizes=client_sizes,
            clip_norm=clip_norm,
            noise_multiplier=noise_multiplier,
            generator=generator,
        )
        global_model.load_state_dict(private_state)
        try:
            metrics = evaluate_model(global_model, test_features, test_labels)
        except ValueError as exc:
            metrics = {"error": str(exc)}
            unstable = True

        history.append({"round": round_index, "test_metrics": metrics})
        if unstable:
            break

    return {
        "setting": setting,
        "noise_multiplier": noise_multiplier,
        "clip_norm": clip_norm,
        "client_label_distributions": [
            {
                "client_id": shard.client_id,
                "size": shard.size,
                **label_distribution(train_labels, shard),
            }
            for shard in shards
        ],
        "history": history,
        "final_metrics": history[-1]["test_metrics"],
        "unstable": unstable,
    }


def run_ldp_tradeoff(
    data_dir: Path,
    output_path: Path,
    num_clients: int,
    rounds: int,
    local_epochs: int,
    batch_size: int,
    learning_rate: float,
    train_sample_size: int,
    clip_norm: float,
    noise_multipliers: list[float],
    seed: int,
) -> dict[str, object]:
    """Evaluate multiple local-DP noise levels for IID and non-IID FedAvg."""

    prepared = prepare_from_disk(DatasetConfig(data_dir=data_dir))
    train_features, train_labels = maybe_subsample_training(
        prepared.x_train.astype("float32"),
        prepared.y_train.astype(int),
        train_sample_size,
        seed,
    )
    test_features = prepared.x_test.astype("float32")
    test_labels = prepared.y_test.astype(int)

    settings = []
    for noise_multiplier in noise_multipliers:
        for setting in ("iid", "non_iid_label_skew"):
            settings.append(
                run_private_setting(
                    setting=setting,
                    train_features=train_features,
                    train_labels=train_labels,
                    test_features=test_features,
                    test_labels=test_labels,
                    num_clients=num_clients,
                    rounds=rounds,
                    local_epochs=local_epochs,
                    batch_size=batch_size,
                    learning_rate=learning_rate,
                    clip_norm=clip_norm,
                    noise_multiplier=noise_multiplier,
                    seed=seed,
                )
            )

    results = {
        "dataset": "UNSW-NB15",
        "task": "binary intrusion detection",
        "method": "FedAvg with local-DP-style update perturbation",
        "privacy_note": (
            "Client updates are clipped and Gaussian noise is added before aggregation. "
            "This demonstrates the privacy-utility tradeoff but does not compute a formal epsilon."
        ),
        "train_sample_size": int(len(train_labels)),
        "test_size": int(len(test_labels)),
        "num_clients": num_clients,
        "rounds": rounds,
        "local_epochs": local_epochs,
        "clip_norm": clip_norm,
        "noise_multipliers": noise_multipliers,
        "settings": settings,
    }

    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(results, indent=2), encoding="utf-8")
    return results


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--data-dir", type=Path, default=DatasetConfig().data_dir)
    parser.add_argument("--output-path", type=Path, default=Path("outputs/ci3203-ldp/ldp_metrics.json"))
    parser.add_argument("--num-clients", type=int, default=5)
    parser.add_argument("--rounds", type=int, default=3)
    parser.add_argument("--local-epochs", type=int, default=1)
    parser.add_argument("--batch-size", type=int, default=512)
    parser.add_argument("--learning-rate", type=float, default=1e-3)
    parser.add_argument("--train-sample-size", type=int, default=20000)
    parser.add_argument("--clip-norm", type=float, default=1.0)
    parser.add_argument("--noise-multipliers", type=float, nargs="+", default=[0.0, 0.25, 0.5, 1.0])
    parser.add_argument("--seed", type=int, default=42)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    results = run_ldp_tradeoff(
        data_dir=args.data_dir,
        output_path=args.output_path,
        num_clients=args.num_clients,
        rounds=args.rounds,
        local_epochs=args.local_epochs,
        batch_size=args.batch_size,
        learning_rate=args.learning_rate,
        train_sample_size=args.train_sample_size,
        clip_norm=args.clip_norm,
        noise_multipliers=args.noise_multipliers,
        seed=args.seed,
    )
    compact = {
        f"{item['setting']} noise={item['noise_multiplier']}": item["final_metrics"]
        for item in results["settings"]
    }
    print(json.dumps(compact, indent=2))


if __name__ == "__main__":
    main()
